...
# Config
# path to the trained model generated by yolo once training is complete
YOLO_WEIGHTS = "runs/segment/models/yolov8_singleclass/weights/best.pt"
# DINO's embeddings
...
def identify():
    ...
    try:
        ...
        print(f"Render path: {match['render_path']}")
...
def render_image():
    ...
    print(f"Render requested: {path}")

    if not path:
        return "No path provided", 400

    # resolve to absolute path — handles both relative and absolute paths
    # and normalises Windows backslashes if present
    ...
    if not abs_path.exists():
        ...
        print(f"Trying cwd relative: {cwd_path}")
        if cwd_path.exists():
            abs_path = cwd_path
        else:
            print(f"[ERROR] Render not found at either path")
