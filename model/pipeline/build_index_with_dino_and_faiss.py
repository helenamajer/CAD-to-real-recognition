...
EMBEDDINGS = MODEL_DATA_DIR / "embeddings"
EMBEDDINGS.mkdir(exist_ok=True)

INDEX_PATH = (EMBEDDINGS / "index").with_suffix(".faiss")
META_PATH = (EMBEDDINGS / "metadata").with_suffix(".json")
# tracks which renders are already indexed
PROCESSED_PATH = (EMBEDDINGS / "processed").with_suffix(".json")
...
if INDEX_PATH.exists() and META_PATH.exists() and PROCESSED_PATH.exists():
    ...
    metadata = json.load(open(str(META_PATH), encoding="utf-8"))
    processed = set(json.load(open(str(PROCESSED_PATH)), encoding="utf-8"))  # set of already-indexed render paths
    ...
    # 768 = DINOv2-base embedding dimension
